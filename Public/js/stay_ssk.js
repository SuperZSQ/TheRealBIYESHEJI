...
(function($){
    stay = {
        ...
        onblur:function () {
            var startTime = stay.start.val().replace(/-/g,"/");
            var endTime = stay.end.val().replace(/-/g,"/");
            var price = $('#single_price').val();
            startTime = new Date(startTime);
            endTime = new Date(endTime);
            var num = (endTime-startTime)/(60*60*24*1000);
            $('#price_order_total').html(price * num);
        }
    }
    stay.init();
})(jQuery);
